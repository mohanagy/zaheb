...
class SupportTicketChat extends Component {
  ...
  componentDidMount =async () => {
    const { actions:{ getCustomerServiceById },userData:{ selectedSupportTicket } ,navigation:{ navigate } } = this.props
  ...
  handleSendMessage =async (id,message) => {
    const { actions:{ sendSupportTicketReply } } = this.props
  ...
  render() {
    const { userData:{ supportTicketConversation,selectedSupportTicket,isFetching } } = this.props
    ...
    return (
      ...
      >
        ...
        <ScrollContainer>
          ...
          <Group><Conversation conversation={supportTicketConversation} /></Group>
        </ScrollContainer>
        <Group style={{ flexDirection: 'row', backgroundColor: '#1E1E1E', padding: 10 }}>
          ...
          />
          <FontAwesome5 name="telegram" size={50} style={{ color: '#FFF', marginLeft: 20 }} onPress={() => this.handleSendMessage(selectedSupportTicket,message)} />
        </Group>
      </Group>
    )
  }
